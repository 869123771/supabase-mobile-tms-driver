...
- Prefer Supabase MCP `search_docs`, `execute_sql`, and `get_advisors` for database work. Verify every DDL/DML change with a read query.
- If Supabase MCP tools are unavailable, check `https://mcp.supabase.com/mcp` reachability. A `401` means the hosted MCP server is reachable; authenticate with Supabase OAuth and reload or start a new Codex task. Use the Supabase CLI only as a fallback.
